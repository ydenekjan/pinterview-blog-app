...
const store = createStore({
  ...
  mutations: {
    ...
    editPostDetails(state, [oldPostSlug, newPost]) {
      let postIdx = state.posts.findIndex((post) => post.slug === oldPostSlug);

      state.posts[postIdx] = newPost;
    },

    removePost(state, postSlug) {
      let postIdx = state.posts.findIndex((post) => post.slug === postSlug);

      state.posts.splice(postIdx, 1);
    },

    returnPost(state, slug) {
  ...
  actions: {
    ...
    async findPost({ commit }, slug) {
      commit("returnPost", slug);
    },
  },
